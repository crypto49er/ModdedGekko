...
var strat = {
	...
	check: function(candle)
	{
		...
		{
			rsi = BEAR_RSI.result;
			let rsi_hi = config.RSI_BULL_BEAR_ADX_TF.BEAR.high,
				rsi_low = config.RSI_BULL_BEAR_ADX_TF.BEAR.low;
			
			// ADX trend strength?
			if( adx > config.RSI_BULL_BEAR_ADX_TF.ADX.high ) rsi_hi = rsi_hi + this.BEAR_MOD_high;
			else if( adx < config.RSI_BULL_BEAR_ADX_TF.ADX.low ) rsi_low = rsi_low + this.BEAR_MOD_low;
			...
			if( rsi > rsi_hi && advised){
				...
				config.RSI_BULL_BEAR_ADX_TF.BEAR.high + '\n\nBuy Price: ' + store.buyPrice + 
				...
				config.RSI_BULL_BEAR_ADX_TF.BEAR.low + '\n\nBuy Price: ' + candle.close;
		...
		{
			rsi = BULL_RSI.result;
			let rsi_hi = config.RSI_BULL_BEAR_ADX_TF.BULL.high,
				rsi_low = config.RSI_BULL_BEAR_ADX_TF.BULL.low;
			
			// ADX trend strength?
			if( adx > config.RSI_BULL_BEAR_ADX_TF.ADX.high ) rsi_hi = rsi_hi + this.BULL_MOD_high;		
			else if( adx < config.RSI_BULL_BEAR_ADX_TF.ADX.low ) rsi_low = rsi_low + this.BULL_MOD_low;
			...
			if( rsi > rsi_hi && advised) {
				...
				config.RSI_BULL_BEAR_ADX_TF.BULL.high + '\n\nBuy Price: ' + store.buyPrice + 
				'\nSell Price: ' + candle.close;
				log.remote(message);
				this.short();
				advised = false;
				...
				config.RSI_BULL_BEAR_ADX_TF.BULL.low + '\n\nBuy Price: ' + candle.close;
				log.remote(message);
				store.buyPrice = candle.close;
				this.long();
				advised = true;
	...
	onCommand: function(cmd){
        ...
        if (command == 'settings') {
            ...
            "RSI Bull High = " + config.RSI_BULL_BEAR_ADX_TF.BULL.high +
            "\nRSI Bull Low = " + config.RSI_BULL_BEAR_ADX_TF.BULL.low +
            "\nRSI Bear High = " + config.RSI_BULL_BEAR_ADX_TF.BEAR.high +
            "\nRSI Bear Low = " + config.RSI_BULL_BEAR_ADX_TF.BEAR.low +
